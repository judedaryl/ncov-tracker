...
export class CaseChartComponent implements OnInit {
  @Input() set caseStatistics(accumulations: Accumulation[][]) {
    if (
      accumulations != null &&
      typeof accumulations !== "undefined" &&
      typeof this.chart !== "undefined"
    ) {
      this.chartOptions.series = this.buildSeries(
        accumulations[0],
        accumulations[1],
        accumulations[2]
      );
    ...
    const caseSeries: Highcharts.SeriesOptionsType = {
      ...
      data: (total || [])
        .map(({ accumulator, value }) => ({
          accumulator: new Date(accumulator),
          value,
        }))
        .filter(({ accumulator }) => accumulator.getTime() > now.getTime())
        .sort((a, b) => a.accumulator.getTime() - b.accumulator.getTime())
