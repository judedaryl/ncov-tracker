...
import {
  ChartsAccumulatedQuery,
  Accumulation,
} from "src/app/graphql/charts-accumulated.query";
import { StatisticsQuery } from "src/app/graphql/statistics.query";
import {
  DistributionQuery,
  AgeGroupDistribution,
} from "src/app/graphql/distribution.query";
...
export class LandingComponent implements OnInit, OnDestroy {
  ...
  setPullInterval() {
    this.caseStatistics$ = interval(1000000).pipe(
      ...
      map(({ data }) =>
        data ? [data.total, data.recovered, data.died] : [[], [], []]
      )
    );
    ...
    this.regionSubscription = this.apollo
      .watchQuery<RegionQuery>({
        query: regionQuery,
      })
      .valueChanges.pipe(
        mergeMap((q) => {
    ...
    this.provinceSubscription = this.regionControl.valueChanges
      .pipe(
        startWith(""),
        mergeMap((region) => {
    ...
    this.citySubscription = combineLatest(
      ...
      this.provinceControl.valueChanges.pipe(startWith(""))
    )
      .pipe(
        mergeMap((val) => {
          this.cityControl.reset("");
